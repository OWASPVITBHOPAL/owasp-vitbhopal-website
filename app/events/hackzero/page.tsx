import type { Metadata } from "next";
import type { ReactNode } from "react";
import Link from "next/link";
import localFont from "next/font/local";
import { Bebas_Neue } from "next/font/google";
import { notFound } from "next/navigation";
import { upCommingEvents } from "@/Content/Events";
import {
  extraFooterItems,
  footerItems,
  socialMediaLinks,
} from "@/Content/LayoutElements";
import { Container } from "@/components/ui/container";
import HackzeroNavbar from "@/components/layout/hackzero-navbar";

const EVENT_SLUG = "hackzero";
const staticEvent = upCommingEvents.find((item) => item.slug === EVENT_SLUG);

const specialAgent = localFont({
  src: "../../../public/events/HackZero/specialagent.ttf",
  variable: "--font-special-agent",
  display: "swap",
});

const bebasNeue = Bebas_Neue({
  weight: "400",
  subsets: ["latin"],
  variable: "--font-bebas-neue",
  display: "swap",
});

const SocialIcon = ({ name, size = 20 }: { name: string; size?: number }) => {
  const iconProps = {
    width: size,
    height: size,
    fill: "currentColor",
    viewBox: "0 0 24 24",
  };

  switch (name) {
    case "Instagram":
      return (
        <svg {...iconProps}>
          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
        </svg>
      );
    case "LinkedIn":
      return (
        <svg {...iconProps}>
          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
        </svg>
      );
    case "GitHub":
      return (
        <svg {...iconProps}>
          <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
        </svg>
      );
    case "Discord":
      return (
        <svg {...iconProps}>
          <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418Z" />
        </svg>
      );
    case "Twitter":
      return (
        <svg {...iconProps}>
          <path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z" />
        </svg>
      );
    case "YouTube":
      return (
        <svg {...iconProps}>
          <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
        </svg>
      );
    default:
      return <span className="text-xs font-bold">{name.charAt(0)}</span>;
  }
};

const SectionTitle = ({
  title,
  subtitle,
}: {
  title: string;
  subtitle?: string;
}) => (
  <div className="mb-5">
    <h2 className="text-3xl md:text-4xl tracking-wide text-white [font-family:var(--font-bebas-neue)]">
      {title}
    </h2>
    {subtitle ? (
      <p className="mt-1.5 text-sm md:text-base text-white/65">{subtitle}</p>
    ) : null}
  </div>
);

const Heading = ({ children }: { children: ReactNode }) => (
  <div className="bg-black px-2 w-fit text-white -mb-4 uppercase text-3xl font-normal tracking-[1px] [font-family:var(--font-bebas-neue)]">
    {children}
  </div>
);

export const metadata: Metadata = {
  title: staticEvent?.title ?? "Event Details",
  description: staticEvent?.description ?? "Event details page",
};

export const revalidate = 3600; // ISR: revalidate every hour

export default function EventDetailsPage() {
  const event = staticEvent;

  if (!event) {
    notFound();
  }

  const mobileHeroImage = event.headerSmallImg ?? event.imgUrl;
  const desktopHeroImage = event.headerLargeImg ?? event.imgUrl;
  const ctaButtonClassName =
    "flex h-10 w-auto px-7 justify-center items-center rounded-2xl bg-white font-bold text-sm         inline-flex items-center rounded-full bg-linear-to-r from-[#B1022B] to-[#660010]";

  return (
    <div
      className={`${specialAgent.variable} ${bebasNeue.variable} w-full relative overflow-hidden min-h-screen bg-linear-to-b from-[#B1022B] to-[#660010]`}
      style={{
        backgroundImage:
          "url(/events/HackZero/hackzero-bg-red-overlay.png), linear-gradient(to bottom, #B1022B, #660010)",
        backgroundRepeat: "repeat, no-repeat",
        backgroundSize: "512px 512px, 100% 100%",
        backgroundPosition: "0 0, 0 0",
        backgroundAttachment: "fixed, scroll",
      }}
    >
      <div
        className={`${specialAgent.variable} ${bebasNeue.variable} w-full relative overflow-hidden min-h-[54vh] sm:min-h-[62vh] md:h-[80vh] bg-[#0F0F0F] [clip-path:polygon(0%_0%,100%_0%,100%_94%,92%_94.3%,84%_95.1%,76%_96.1%,68%_97.1%,60%_98.2%,54%_99.1%,50%_99.7%,46%_99.1%,40%_98.2%,32%_97.1%,24%_96.1%,16%_95.1%,8%_94.3%,0%_94%)] md:[clip-path:polygon(0%_0%,100%_0%,100%_89.36%,92.13%_89.79%,84.52%_90.68%,71.45%_93.14%,60.74%_96.47%,50.17%_99.99%,38.8%_96.29%,27.41%_93.1%,14.86%_90.61%,7.41%_89.79%,0%_89.34%)]`}
        style={{
          backgroundImage: "url(/events/hackzero/hackzero-bg-overlay.png)",
          backgroundRepeat: "repeat",
          backgroundSize: "512px 512px",
          backgroundPosition: "0 0",
          backgroundAttachment: "fixed",
        }}
      >
        <HackzeroNavbar />
        <div className="h-full w-full">
          <Container className="relative h-full w-full">
            <div className="flex flex-col h-full w-full items-center pt-16 md:pt-24">
              <div className="text-white -mb-3 uppercase text-lg sm:text-2xl md:text-3xl font-normal tracking-[1px] [font-family:var(--font-bebas-neue)]">
                OWASP VIT Bhopal presents
              </div>
              <div className="w-full max-w-[90vw]  md:max-w-200">
                <svg
                  className="h-auto w-full"
                  viewBox="0 0 930 212"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M4.4 190V33.36H34.76V73.84H44.22V33.36H74.58V190H44.22V101.34H34.76V190H4.4ZM124.823 133.24H134.283V65.7C134.283 62.4733 132.743 60.86 129.663 60.86C126.436 60.86 124.823 62.4733 124.823 65.7V133.24ZM94.4625 60.86C94.4625 54.4067 95.1958 49.3467 96.6625 45.68C98.1292 42.0133 100.329 39.3 103.263 37.54C106.196 35.6333 109.863 34.46 114.263 34.02C118.663 33.58 123.796 33.36 129.663 33.36C135.529 33.36 140.589 33.58 144.843 34.02C149.243 34.46 152.909 35.6333 155.843 37.54C158.776 39.3 160.976 42.0133 162.443 45.68C163.909 49.3467 164.643 54.4067 164.643 60.86V190H134.283V160.74H124.823V190H94.4625V60.86ZM219.725 33.36H239.525V60.86H219.505C216.718 61.0067 215.178 62.4 214.885 65.04V157.66C214.885 160.887 216.498 162.5 219.725 162.5H239.525V190H219.725C213.858 190 208.725 189.78 204.325 189.34C199.925 188.9 196.258 187.8 193.325 186.04C190.392 184.133 188.192 181.347 186.725 177.68C185.258 174.013 184.525 168.953 184.525 162.5V60.86C184.525 54.5533 185.258 49.5667 186.725 45.9C188.192 42.2333 190.318 39.4467 193.105 37.54C196.038 35.6333 199.705 34.46 204.105 34.02C208.505 33.58 213.712 33.36 219.725 33.36ZM299.154 86.16V33.36H329.514V82.86C329.514 89.1667 328.56 94.0067 326.654 97.38C324.747 100.753 322.18 103.247 318.954 104.86C322.327 106.473 324.894 109.113 326.654 112.78C328.56 116.3 329.514 121.58 329.514 128.62V190H299.154V123.34C299.154 120.26 297.54 118.647 294.314 118.5H289.694V190H259.334V33.36H289.694V91H295.194C297.834 90.8533 299.154 89.24 299.154 86.16Z"
                    fill="url(#paint0_radial_194_70)"
                  />
                  <path
                    d="M396.22 128.84V125.34H392.72V128.84H396.22ZM426.58 128.84H430.08V125.34H426.58V128.84ZM426.58 190V193.5H430.08V190H426.58ZM356.4 190H352.9V193.5H356.4V190ZM356.84 127.3L360.332 127.533V127.533L356.84 127.3ZM358.38 118.72L361.44 120.42L361.744 119.872L361.839 119.252L358.38 118.72ZM362.78 108.82L360.113 106.553L360.108 106.559L360.103 106.566L362.78 108.82ZM371.58 100.9L373.579 103.773L373.62 103.744L373.66 103.715L371.58 100.9ZM382.36 94.08L384.019 97.1616L384.04 97.1507L384.06 97.1395L382.36 94.08ZM393.58 87.04L395.603 89.8961L395.687 89.8369L395.766 89.773L393.58 87.04ZM395.34 83.96L392.051 82.7639L392.016 82.8582L391.988 82.9543L395.34 83.96ZM396.22 60.86H399.72V57.36H396.22V60.86ZM386.76 60.86V57.36H383.26V60.86H386.76ZM386.76 83.96V87.46H390.26V83.96H386.76ZM356.4 83.96H352.9V87.46H356.4V83.96ZM356.4 33.36V29.86H352.9V33.36H356.4ZM426.58 33.36H430.08V29.86H426.58V33.36ZM426.14 92.98L422.659 92.6136L422.65 92.6996L422.645 92.7859L426.14 92.98ZM424.38 99.58L421.357 97.8164L421.341 97.843L421.326 97.8699L424.38 99.58ZM417.12 108.38L414.951 105.633H414.951L417.12 108.38ZM408.1 114.32L406.424 111.247L406.394 111.264L406.364 111.281L408.1 114.32ZM398.64 119.82L400.316 122.893L400.369 122.864L400.421 122.833L398.64 119.82ZM389.62 126.64L387.145 124.165L386.799 124.512L386.56 124.94L389.62 126.64ZM386.98 135L383.497 134.652L383.489 134.734L383.485 134.816L386.98 135ZM386.76 162.5H383.26V166H386.76V162.5ZM396.22 162.5V166H399.72V162.5H396.22ZM396.22 128.84V132.34H426.58V128.84V125.34H396.22V128.84ZM426.58 128.84H423.08V190H426.58H430.08V128.84H426.58ZM426.58 190V186.5H356.4V190V193.5H426.58V190ZM356.4 190H359.9V140.94H356.4H352.9V190H356.4ZM356.4 140.94H359.9C359.9 136.319 360.044 131.851 360.332 127.533L356.84 127.3L353.348 127.067C353.049 131.549 352.9 136.174 352.9 140.94H356.4ZM356.84 127.3L360.332 127.533C360.474 125.403 360.665 123.729 360.89 122.473C361.129 121.14 361.362 120.56 361.44 120.42L358.38 118.72L355.32 117.02C354.665 118.2 354.274 119.71 354 121.237C353.712 122.841 353.499 124.797 353.348 127.067L356.84 127.3ZM358.38 118.72L361.839 119.252C362.334 116.035 363.549 113.34 365.457 111.074L362.78 108.82L360.103 106.566C357.318 109.873 355.599 113.779 354.921 118.188L358.38 118.72ZM362.78 108.82L365.447 111.087C367.732 108.398 370.437 105.959 373.579 103.773L371.58 100.9L369.581 98.0268C365.977 100.534 362.815 103.375 360.113 106.553L362.78 108.82ZM371.58 100.9L373.66 103.715C376.896 101.323 380.348 99.1386 384.019 97.1616L382.36 94.08L380.701 90.9983C376.746 93.128 373.011 95.49 369.5 98.0854L371.58 100.9ZM382.36 94.08L384.06 97.1395C388.126 94.8807 391.974 92.4666 395.603 89.8961L393.58 87.04L391.557 84.1839C388.146 86.6 384.514 88.8793 380.66 91.0204L382.36 94.08ZM393.58 87.04L395.766 89.773C397.305 88.542 398.165 86.7224 398.692 84.9657L395.34 83.96L391.988 82.9543C391.823 83.5033 391.661 83.8671 391.531 84.0935C391.402 84.3193 391.339 84.3503 391.394 84.307L393.58 87.04ZM395.34 83.96L398.629 85.1561C399.37 83.1193 399.72 80.9497 399.72 78.68H396.22H392.72C392.72 80.2236 392.483 81.574 392.051 82.7639L395.34 83.96ZM396.22 78.68H399.72V60.86H396.22H392.72V78.68H396.22ZM396.22 60.86V57.36H386.76V60.86V64.36H396.22V60.86ZM386.76 60.86H383.26V83.96H386.76H390.26V60.86H386.76ZM386.76 83.96V80.46H356.4V83.96V87.46H386.76V83.96ZM356.4 83.96H359.9V33.36H356.4H352.9V83.96H356.4ZM356.4 33.36V36.86H426.58V33.36V29.86H356.4V33.36ZM426.58 33.36H423.08V83.74H426.58H430.08V33.36H426.58ZM426.58 83.74H423.08C423.08 87.0147 422.937 89.9702 422.659 92.6136L426.14 92.98L429.621 93.3464C429.929 90.4165 430.08 87.212 430.08 83.74H426.58ZM426.14 92.98L422.645 92.7859C422.521 95.0319 422.034 96.6558 421.357 97.8164L424.38 99.58L427.403 101.344C428.78 98.9842 429.466 96.2081 429.635 93.1741L426.14 92.98ZM424.38 99.58L421.326 97.8699C419.457 101.208 417.318 103.764 414.951 105.633L417.12 108.38L419.289 111.127C422.495 108.596 425.197 105.285 427.434 101.29L424.38 99.58ZM417.12 108.38L414.951 105.633C412.325 107.706 409.484 109.578 406.424 111.247L408.1 114.32L409.776 117.393C413.17 115.542 416.342 113.454 419.289 111.127L417.12 108.38ZM408.1 114.32L406.364 111.281C403.268 113.05 400.1 114.892 396.859 116.807L398.64 119.82L400.421 122.833C403.633 120.935 406.772 119.11 409.836 117.359L408.1 114.32ZM398.64 119.82L396.964 116.747C393.423 118.679 390.151 121.159 387.145 124.165L389.62 126.64L392.095 129.115C394.662 126.548 397.403 124.481 400.316 122.893L398.64 119.82ZM389.62 126.64L386.56 124.94C384.868 127.987 383.839 131.232 383.497 134.652L386.98 135L390.463 135.348C390.707 132.901 391.439 130.573 392.68 128.34L389.62 126.64ZM386.98 135L383.485 134.816C383.337 137.633 383.26 139.974 383.26 141.82H386.76H390.26C390.26 140.146 390.33 137.94 390.475 135.184L386.98 135ZM386.76 141.82H383.26V162.5H386.76H390.26V141.82H386.76ZM386.76 162.5V166H396.22V162.5V159H386.76V162.5ZM396.22 162.5H399.72V128.84H396.22H392.72V162.5H396.22ZM497.277 33.36H500.777V29.86H497.277V33.36ZM497.277 60.86V64.36H500.777V60.86H497.277ZM477.037 60.86V57.36H473.537V60.86H477.037ZM477.037 73.84H473.537V77.34H477.037V73.84ZM497.277 73.84H500.777V70.34H497.277V73.84ZM497.277 101.34V104.84H500.777V101.34H497.277ZM477.037 101.34V97.84H473.537V101.34H477.037ZM477.037 162.5H473.537V166H477.037V162.5ZM497.277 162.5H500.777V159H497.277V162.5ZM497.277 190V193.5H500.777V190H497.277ZM446.677 190H443.177V193.5H446.677V190ZM446.677 33.36V29.86H443.177V33.36H446.677ZM497.277 33.36H493.777V60.86H497.277H500.777V33.36H497.277ZM497.277 60.86V57.36H477.037V60.86V64.36H497.277V60.86ZM477.037 60.86H473.537V73.84H477.037H480.537V60.86H477.037ZM477.037 73.84V77.34H497.277V73.84V70.34H477.037V73.84ZM497.277 73.84H493.777V101.34H497.277H500.777V73.84H497.277ZM497.277 101.34V97.84H477.037V101.34V104.84H497.277V101.34ZM477.037 101.34H473.537V162.5H477.037H480.537V101.34H477.037ZM477.037 162.5V166H497.277V162.5V159H477.037V162.5ZM497.277 162.5H493.777V190H497.277H500.777V162.5H497.277ZM497.277 190V186.5H446.677V190V193.5H497.277V190ZM446.677 190H450.177V33.36H446.677H443.177V190H446.677ZM446.677 33.36V36.86H497.277V33.36V29.86H446.677V33.36ZM516.974 190H513.474V193.5H516.974V190ZM516.974 33.36V29.86H513.474V33.36H516.974ZM567.354 34.02L566.994 37.5014L567.006 37.5026L567.354 34.02ZM578.354 37.54L580.262 34.6054V34.6054L578.354 37.54ZM584.954 45.9L588.204 44.6001V44.6001L584.954 45.9ZM584.294 97.38L581.247 95.6578V95.6578L584.294 97.38ZM576.594 104.86L575.029 101.729L568.628 104.93L575.084 108.017L576.594 104.86ZM584.294 112.78L581.139 114.295L581.176 114.372L581.217 114.447L584.294 112.78ZM587.154 190V193.5H590.654V190H587.154ZM556.794 190H553.294V193.5H556.794V190ZM551.954 118.5L552.113 115.004L552.034 115H551.954V118.5ZM547.334 118.5V115H543.834V118.5H547.334ZM547.334 190V193.5H550.834V190H547.334ZM551.954 60.86L552.113 57.3636L552.034 57.36H551.954V60.86ZM547.334 60.86V57.36H543.834V60.86H547.334ZM547.334 91H543.834V94.5H547.334V91ZM552.834 91V94.5H552.931L553.028 94.4946L552.834 91ZM516.974 190H520.474V33.36H516.974H513.474V190H516.974ZM516.974 33.36V36.86H552.174V33.36V29.86H516.974V33.36ZM552.174 33.36V36.86C557.961 36.86 562.895 37.0774 566.994 37.5014L567.354 34.02L567.714 30.5386C563.307 30.0826 558.12 29.86 552.174 29.86V33.36ZM567.354 34.02L567.006 37.5026C570.993 37.9013 574.089 38.9422 576.447 40.4745L578.354 37.54L580.262 34.6054C576.752 32.3244 572.515 31.0186 567.702 30.5374L567.354 34.02ZM578.354 37.54L576.447 40.4745C578.698 41.9382 580.47 44.1137 581.705 47.1999L584.954 45.9L588.204 44.6001C586.505 40.353 583.877 36.9552 580.262 34.6054L578.354 37.54ZM584.954 45.9L581.705 47.1999C582.937 50.2804 583.654 54.7643 583.654 60.86H587.154H590.654C590.654 54.3424 589.905 48.8529 588.204 44.6001L584.954 45.9ZM587.154 60.86H583.654V82.86H587.154H590.654V60.86H587.154ZM587.154 82.86H583.654C583.654 88.9018 582.726 93.0418 581.247 95.6578L584.294 97.38L587.341 99.1022C589.676 94.9715 590.654 89.4315 590.654 82.86H587.154ZM584.294 97.38L581.247 95.6578C579.657 98.4719 577.586 100.451 575.029 101.729L576.594 104.86L578.159 107.99C582.056 106.042 585.118 103.035 587.341 99.1022L584.294 97.38ZM576.594 104.86L575.084 108.017C577.673 109.256 579.692 111.28 581.139 114.295L584.294 112.78L587.45 111.265C585.377 106.947 582.262 103.691 578.104 101.703L576.594 104.86ZM584.294 112.78L581.217 114.447C582.709 117.201 583.654 121.781 583.654 128.62H587.154H590.654C590.654 121.379 589.693 115.399 587.372 111.113L584.294 112.78ZM587.154 128.62H583.654V190H587.154H590.654V128.62H587.154ZM587.154 190V186.5H556.794V190V193.5H587.154V190ZM556.794 190H560.294V123.34H556.794H553.294V190H556.794ZM556.794 123.34H560.294C560.294 121.206 559.726 119.012 558.059 117.345C556.419 115.706 554.237 115.1 552.113 115.004L551.954 118.5L551.795 121.996C552.334 122.021 552.676 122.102 552.876 122.174C552.972 122.209 553.031 122.241 553.063 122.261C553.094 122.28 553.106 122.292 553.109 122.295C553.112 122.298 553.118 122.303 553.127 122.319C553.137 122.335 553.158 122.373 553.182 122.443C553.231 122.587 553.294 122.868 553.294 123.34H556.794ZM551.954 118.5V115H547.334V118.5V122H551.954V118.5ZM547.334 118.5H543.834V190H547.334H550.834V118.5H547.334ZM547.334 190V186.5H516.974V190V193.5H547.334V190ZM556.794 86.16H560.294V65.7H556.794H553.294V86.16H556.794ZM556.794 65.7H560.294C560.294 63.5658 559.726 61.3717 558.059 59.7051C556.419 58.0655 554.237 57.4602 552.113 57.3636L551.954 60.86L551.795 64.3564C552.334 64.3809 552.676 64.4617 552.876 64.5343C552.972 64.5695 553.031 64.6011 553.063 64.6208C553.094 64.6399 553.106 64.6519 553.109 64.6549C553.112 64.6576 553.118 64.6633 553.127 64.6788C553.137 64.6952 553.158 64.7331 553.182 64.8028C553.231 64.9472 553.294 65.2275 553.294 65.7H556.794ZM551.954 60.86V57.36H547.334V60.86V64.36H551.954V60.86ZM547.334 60.86H543.834V91H547.334H550.834V60.86H547.334ZM547.334 91V94.5H552.834V91V87.5H547.334V91ZM552.834 91L553.028 94.4946C555.048 94.3824 557.073 93.6562 558.513 91.8963C559.879 90.2264 560.294 88.1351 560.294 86.16H556.794H553.294C553.294 87.2649 553.049 87.5202 553.095 87.4637C553.126 87.4266 553.161 87.4098 553.145 87.4167C553.113 87.431 552.967 87.4872 552.64 87.5054L552.834 91ZM642.017 60.86L642.176 57.3636L641.93 57.3524L641.685 57.3758L642.017 60.86ZM609.237 45.9L605.987 44.6001V44.6001L609.237 45.9ZM615.617 37.54L613.709 34.6054L613.675 34.628L613.64 34.6514L615.617 37.54ZM626.617 34.02L626.268 30.5374V30.5374L626.617 34.02ZM657.417 34.02L657.057 37.5014L657.068 37.5026L657.417 34.02ZM668.417 37.54L670.324 34.6054V34.6054L668.417 37.54ZM675.017 45.9L678.266 44.6001V44.6001L675.017 45.9ZM675.017 177.68L678.266 178.98V178.98L675.017 177.68ZM668.417 186.04L670.217 189.041L670.271 189.009L670.324 188.975L668.417 186.04ZM657.417 189.34L657.068 185.857L657.057 185.859L657.417 189.34ZM626.837 189.34L627.185 185.857V185.857L626.837 189.34ZM615.837 186.04L613.929 188.975L613.982 189.009L614.036 189.041L615.837 186.04ZM609.237 177.68L605.987 178.98V178.98L609.237 177.68ZM646.857 65.7H650.357C650.357 63.5658 649.788 61.3717 648.122 59.7051C646.482 58.0655 644.3 57.4602 642.176 57.3636L642.017 60.86L641.858 64.3564C642.396 64.3809 642.739 64.4617 642.938 64.5343C643.035 64.5695 643.094 64.6011 643.126 64.6208C643.157 64.6399 643.169 64.6519 643.172 64.6549C643.175 64.6576 643.18 64.6633 643.19 64.6788C643.2 64.6952 643.22 64.7331 643.244 64.8028C643.294 64.9472 643.357 65.2275 643.357 65.7H646.857ZM642.017 60.86L641.685 57.3758C639.631 57.5714 637.562 58.2581 636.02 59.8733C634.452 61.5157 633.897 63.6257 633.897 65.7H637.397H640.897C640.897 65.2668 640.954 65.0069 641 64.8703C641.042 64.7478 641.077 64.7134 641.083 64.7067C641.093 64.697 641.15 64.6373 641.331 64.5628C641.522 64.4837 641.845 64.3922 642.349 64.3442L642.017 60.86ZM637.397 65.7H633.897V157.66H637.397H640.897V65.7H637.397ZM637.397 157.66H633.897C633.897 159.813 634.437 162.07 636.132 163.765C637.826 165.459 640.084 166 642.237 166V162.5V159C641.163 159 641 158.734 641.082 158.815C641.163 158.896 640.897 158.734 640.897 157.66H637.397ZM642.237 162.5V166C644.359 166 646.583 165.436 648.233 163.707C649.855 162.008 650.357 159.773 650.357 157.66H646.857H643.357C643.357 158.773 643.088 158.959 643.17 158.873C643.281 158.757 643.195 159 642.237 159V162.5ZM646.857 157.66H650.357V65.7H646.857H643.357V157.66H646.857ZM607.037 60.86H610.537C610.537 54.7643 611.254 50.2804 612.486 47.1999L609.237 45.9L605.987 44.6001C604.286 48.8529 603.537 54.3424 603.537 60.86H607.037ZM609.237 45.9L612.486 47.1999C613.74 44.0655 615.471 41.8807 617.593 40.4286L615.617 37.54L613.64 34.6514C610.189 37.0126 607.667 40.4012 605.987 44.6001L609.237 45.9ZM615.617 37.54L617.524 40.4745C619.882 38.9422 622.978 37.9013 626.965 37.5026L626.617 34.02L626.268 30.5374C621.456 31.0186 617.218 32.3244 613.709 34.6054L615.617 37.54ZM626.617 34.02L626.965 37.5026C631.217 37.0775 636.301 36.86 642.237 36.86V33.36V29.86C636.145 29.86 630.817 30.0825 626.268 30.5374L626.617 34.02ZM642.237 33.36V36.86C648.024 36.86 652.958 37.0774 657.057 37.5014L657.417 34.02L657.777 30.5386C653.369 30.0826 648.183 29.86 642.237 29.86V33.36ZM657.417 34.02L657.068 37.5026C661.056 37.9013 664.152 38.9422 666.509 40.4745L668.417 37.54L670.324 34.6054C666.815 32.3244 662.578 31.0186 657.765 30.5374L657.417 34.02ZM668.417 37.54L666.509 40.4745C668.761 41.9382 670.533 44.1137 671.767 47.1999L675.017 45.9L678.266 44.6001C676.568 40.353 673.939 36.9552 670.324 34.6054L668.417 37.54ZM675.017 45.9L671.767 47.1999C672.999 50.2804 673.717 54.7643 673.717 60.86H677.217H680.717C680.717 54.3424 679.968 48.8529 678.266 44.6001L675.017 45.9ZM677.217 60.86H673.717V162.5H677.217H680.717V60.86H677.217ZM677.217 162.5H673.717C673.717 168.755 672.996 173.307 671.767 176.38L675.017 177.68L678.266 178.98C679.97 174.72 680.717 169.152 680.717 162.5H677.217ZM675.017 177.68L671.767 176.38C670.533 179.466 668.761 181.642 666.509 183.105L668.417 186.04L670.324 188.975C673.939 186.625 676.568 183.227 678.266 178.98L675.017 177.68ZM668.417 186.04L666.616 183.039C664.235 184.467 661.099 185.454 657.068 185.857L657.417 189.34L657.765 192.823C662.534 192.346 666.732 191.133 670.217 189.041L668.417 186.04ZM657.417 189.34L657.057 185.859C652.958 186.283 648.024 186.5 642.237 186.5V190V193.5C648.183 193.5 653.369 193.277 657.777 192.821L657.417 189.34ZM642.237 190V186.5C636.451 186.5 631.439 186.283 627.185 185.857L626.837 189.34L626.488 192.823C631.034 193.277 636.289 193.5 642.237 193.5V190ZM626.837 189.34L627.185 185.857C623.154 185.454 620.018 184.467 617.637 183.039L615.837 186.04L614.036 189.041C617.522 191.133 621.719 192.346 626.488 192.823L626.837 189.34ZM615.837 186.04L617.744 183.105C615.492 181.642 613.721 179.466 612.486 176.38L609.237 177.68L605.987 178.98C607.686 183.227 610.314 186.625 613.929 188.975L615.837 186.04ZM609.237 177.68L612.486 176.38C611.257 173.307 610.537 168.755 610.537 162.5H607.037H603.537C603.537 169.152 604.283 174.72 605.987 178.98L609.237 177.68ZM607.037 162.5H610.537V60.86H607.037H603.537V162.5H607.037Z"
                    fill="url(#paint1_radial_194_70)"
                  />
                  <path
                    d="M745.32 78.68H715.4V60.86C715.4 54.9933 715.987 50.2267 717.16 46.56C718.48 42.8933 720.313 40.1067 722.66 38.2C725.153 36.2933 728.233 35.0467 731.9 34.46C735.713 33.7267 740.187 33.36 745.32 33.36V78.68ZM804.892 162.5V128.84H835.252V190H765.072V140.94C765.072 136.247 765.219 131.7 765.512 127.3C765.805 122.9 766.319 120.04 767.052 118.72C768.079 114.76 769.839 111.387 772.332 108.6C774.825 105.667 777.685 103.027 780.912 100.68C784.139 98.3333 787.585 96.1333 791.252 94.08C795.065 91.88 798.732 89.5333 802.252 87.04C802.985 86.4533 803.572 85.4267 804.012 83.96C804.599 82.3467 804.892 80.5867 804.892 78.68V65.7C804.892 62.7667 803.352 61.1533 800.272 60.86C797.045 61.0067 795.432 62.62 795.432 65.7V84.18H765.072V60.86C765.072 54.5533 765.805 49.5667 767.272 45.9C768.739 42.2333 770.939 39.4467 773.872 37.54C776.805 35.6333 780.399 34.46 784.652 34.02C789.052 33.58 794.185 33.36 800.052 33.36C805.919 33.36 811.052 33.58 815.452 34.02C819.999 34.46 823.665 35.6333 826.452 37.54C829.385 39.4467 831.585 42.2333 833.052 45.9C834.519 49.5667 835.252 54.5533 835.252 60.86V85.28C835.252 88.36 835.105 91.1467 834.812 93.64C834.665 95.9867 834.079 97.9667 833.052 99.58C830.852 102.953 828.285 105.74 825.352 107.94C822.565 110.14 819.632 112.193 816.552 114.1C813.472 115.86 810.319 117.693 807.092 119.6C804.012 121.507 801.079 123.853 798.292 126.64C796.825 129.28 795.945 132.067 795.652 135C795.505 137.787 795.432 140.06 795.432 141.82V162.5H804.892ZM894.954 65.7C894.954 62.62 893.341 61.0067 890.114 60.86C887.034 61.1533 885.494 62.7667 885.494 65.7V95.84H890.114C895.981 95.84 901.114 96.06 905.514 96.5C909.914 96.94 913.581 98.1133 916.514 100.02C919.448 101.78 921.648 104.493 923.114 108.16C924.581 111.827 925.314 116.887 925.314 123.34V162.5C925.314 168.953 924.581 174.013 923.114 177.68C921.648 181.347 919.448 184.133 916.514 186.04C913.581 187.8 909.914 188.9 905.514 189.34C901.261 189.78 896.201 190 890.334 190C884.468 190 879.334 189.78 874.934 189.34C870.534 188.9 866.868 187.8 863.934 186.04C861.001 184.133 858.801 181.347 857.334 177.68C855.868 174.013 855.134 168.953 855.134 162.5V60.86C855.134 54.5533 855.868 49.5667 857.334 45.9C858.801 42.2333 860.928 39.4467 863.714 37.54C866.648 35.6333 870.314 34.46 874.714 34.02C879.114 33.58 884.321 33.36 890.334 33.36C896.201 33.36 901.261 33.58 905.514 34.02C909.914 34.46 913.581 35.6333 916.514 37.54C919.448 39.4467 921.648 42.2333 923.114 45.9C924.581 49.5667 925.314 54.5533 925.314 60.86V78.46H894.954V65.7ZM885.494 123.34V157.66C885.494 160.887 887.108 162.5 890.334 162.5C893.121 162.5 894.661 161.18 894.954 158.54V127.52C894.808 124.88 893.414 123.487 890.774 123.34H885.494Z"
                    fill="url(#paint2_radial_194_70)"
                  />
                  <defs>
                    <radialGradient
                      id="paint0_radial_194_70"
                      cx="0"
                      cy="0"
                      r="1"
                      gradientUnits="userSpaceOnUse"
                      gradientTransform="translate(166.906 41.692) rotate(90) scale(183.69 289.399)"
                    >
                      <stop stop-color="#B1022B" />
                      <stop offset="1" stop-color="#660010" />
                    </radialGradient>
                    <radialGradient
                      id="paint1_radial_194_70"
                      cx="0"
                      cy="0"
                      r="1"
                      gradientUnits="userSpaceOnUse"
                      gradientTransform="translate(522.404 41.692) rotate(90) scale(183.69 295.464)"
                    >
                      <stop stop-color="#B1022B" />
                      <stop offset="1" stop-color="#660010" />
                    </radialGradient>
                    <radialGradient
                      id="paint2_radial_194_70"
                      cx="0"
                      cy="0"
                      r="1"
                      gradientUnits="userSpaceOnUse"
                      gradientTransform="translate(820.438 41.692) rotate(90) scale(183.69 189.756)"
                    >
                      <stop stop-color="#B1022B" />
                      <stop offset="1" stop-color="#660010" />
                    </radialGradient>
                  </defs>
                </svg>
              </div>
              <div className="text-white border-y-4 py-1 uppercase text-lg sm:text-2xl md:text-3xl font-normal leading-none tracking-[1px] [font-family:var(--font-bebas-neue)]">
                28-29 March 2026
              </div>
              <div className="text-center mt-5 sm:mt-6 md:mt-8">
                <a
                  href={event.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={` ${ctaButtonClassName} text-base sm:text-lg md:text-xl rounded-xl py-4 sm:py-5 md:py-6 hover:ring-1`}
                >
                  Register on Unstop
                </a>
              </div>
            </div>
          </Container>
        </div>
        <svg
          className="pointer-events-none absolute inset-0 w-full h-full overflow-visible"
          preserveAspectRatio="none"
          viewBox="0 0 100 100"
          xmlns="http://www.w3.org/2000/svg"
        >
          <polyline
            className="md:hidden"
            points="0,94 8,94.3 16,95.1 24,96.1 32,97.1 40,98.2 46,99.1 50,99.7 54,99.1 60,98.2 68,97.1 76,96.1 84,95.1 92,94.3 100,94"
            fill="none"
            stroke="#FFFFFF"
            strokeWidth="3.8"
          />
          <polyline
            className="hidden md:block"
            points="0,89.34 7.41,89.79 14.86,90.61 27.41,93.1 38.8,96.29 50.17,99.99 60.74,96.47 71.45,93.14 84.52,90.68 92.13,89.79 100,89.36"
            fill="none"
            stroke="#FFFFFF"
            strokeWidth="3.5"
          />
        </svg>
      </div>

      <Container className="py-10 md:py-14 text-white">
        <div className="grid gap-10 lg:grid-cols-3 lg:items-start">
          <div className="space-y-10 lg:col-span-2">
            <section className="space-y-4">
              <Heading>About</Heading>
              <div className="rounded-2xl p-5 md:p-6">
                <ul className="space-y-2 list-disc pl-6 text-white/90">
                  {event.about.map((point) => (
                    <li key={point}>{point}</li>
                  ))}
                </ul>
              </div>
            </section>

            {/* Timeline section removed as per request */}

            {/* Day 1 & Day 2 Schedule Two Column Section */}
            <section className="space-y-4 mt-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Day 1 */}
                <div className="flex flex-col items-start">
                  <Heading>Day 1 - 28 March</Heading>
                  <div className="flex mt-8 flex-col gap-2 w-full">
                    <div className="rounded-xl bg-[#85011b] p-3 flex justify-between">
                      <div className="text-base font-bold text-white/90">
                        13:30 – 13:45
                      </div>
                      <div className="text-white/80 text-sm">
                        Introduction to the event
                      </div>
                    </div>
                    <div className="rounded-xl bg-[#85011b] p-3 flex justify-between">
                      <div className="text-base font-bold text-white/90">
                        13:45 – 14:00
                      </div>
                      <div className="text-white/80 text-sm">
                        Address by Faculty Coordinator
                      </div>
                    </div>
                    <div className="rounded-xl bg-[#85011b] p-4 flex flex-col gap-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="text-base font-bold text-white/90">
                          14:00 - 15:00
                        </div>
                        <div className="text-white/80 text-sm">
                          Expert session
                        </div>
                      </div>
                      <div className="grid grid-cols-[96px_minmax(0,1fr)] items-start gap-4 sm:grid-cols-[110px_minmax(0,1fr)]">
                        <div className="overflow-hidden rounded-lg border border-white/40 max-h-[112px]">
                          <img
                            src="/events/HackZero/speakers/sourajeet.png"
                            alt="Sourajeet Majumder"
                            className="block h-full w-full object-cover"
                          />
                        </div>
                        <div className="flex min-w-0 flex-col justify-start">
                          <div className="text-white/90 text-sm font-semibold mt-0.5">
                            Session by Sourajeet Majumder{" "}
                            <span className="text-white/60 text-xs">
                              (Security Engineer at CloudSEK)
                            </span>
                          </div>
                          <div className="text-white/80 text-xs mt-0.5">
                            Topic:{" "}
                            <span className="font-bold">
                              Unmasking Cybercriminals: An Intelligence-Led
                              Approach
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="rounded-xl bg-[#85011b] p-3 flex items-start justify-between gap-4">
                      <div className="text-base font-bold text-white/90">
                        15:00 – 15:15
                      </div>
                      <div className="text-white/80 text-sm">Break</div>
                    </div>
                    <div className="rounded-xl bg-[#85011b] p-4 flex flex-col gap-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="text-base font-bold text-white/90">
                          15:15 - 16:15
                        </div>
                        <div className="text-white/80 text-sm">
                          Expert session
                        </div>
                      </div>
                      <div className="grid grid-cols-[96px_minmax(0,1fr)] items-start gap-4 sm:grid-cols-[110px_minmax(0,1fr)]">
                        <div className="overflow-hidden rounded-lg border border-white/40 max-h-[112px]">
                          <img
                            src="/events/HackZero/speakers/prathmesh.png"
                            alt="Prathmesh Dharkar"
                            className="block h-full w-full object-cover"
                          />
                        </div>
                        <div className="flex min-w-0 flex-col justify-start">
                          <div className="text-white/90 text-sm font-semibold mt-0.5">
                            Session by Prathmesh Dharkar{" "}
                            <span className="text-white/60 text-xs">
                              (Security expert and Chapter Leader at BSides
                              Indore)
                            </span>
                          </div>
                          <div className="text-white/80 text-xs mt-0.5">
                            Topic:{" "}
                            <span className="font-bold">
                              Social Engineering: Hacking the Human Mind
                            </span>
                          </div>
                          <a
                            href="https://www.youtube.com/live/QdEsbX0I54M"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="mt-3 inline-flex w-fit items-center rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold text-white transition hover:bg-white/20"
                          >
                            Watch it here
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="rounded-xl bg-[#85011b] p-3 flex items-start justify-between gap-4">
                      <div className="text-base font-bold text-white/90">
                        16:15 – 16:30
                      </div>
                      <div className="text-white/80 text-sm">
                        Closing note and conclusion
                      </div>
                    </div>
                  </div>
                </div>
                {/* Day 2 */}
                <div className="flex flex-col items-start justify-start h-full">
                  <Heading>Day 2 - 29 March</Heading>
                  <div className="rounded-xl bg-[#85011b] mt-8 p-4 flex flex-col gap-2 w-full">
                    <div className="text-base font-bold text-white/90">
                      00:00 – 23:59
                    </div>
                    <div className="text-white/90 text-sm font-semibold">
                      Capture The Flag (CTF)
                    </div>
                    <div className="text-white/80 text-xs">
                      Starts at midnight and runs all day
                    </div>
                    <a
                      href="https://discord.gg/cXWUQk6gS"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="mt-3 inline-flex w-fit items-center rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold text-white transition hover:bg-white/20"
                    >
                      Join on Discord
                    </a>
                  </div>
                </div>
              </div>
            </section>
          </div>

          <aside className="space-y-10 lg:col-span-1 lg:sticky lg:top-6">
            <section className="space-y-4">
              <Heading>Tracks</Heading>
              <div className="rounded-2xl p-5 md:p-6">
                <div className="flex flex-wrap gap-2">
                  {event.tracks.map((track) => (
                    <span
                      key={track}
                      className="rounded-full bg-white/10 px-4 py-1.5 text-sm"
                    >
                      {track}
                    </span>
                  ))}
                </div>
              </div>
            </section>

            <section className="space-y-4">
              <Heading>Challenge Domains</Heading>
              <div className="rounded-2xl p-5 md:p-6">
                <div className="flex flex-wrap gap-2">
                  {event.challengeDomains.map((domain) => (
                    <span
                      key={domain}
                      className="rounded-full bg-white/10 px-4 py-1.5 text-sm"
                    >
                      {domain}
                    </span>
                  ))}
                </div>
              </div>
            </section>
          </aside>
        </div>
        <section className="space-y-4 mt-6">
          <Heading>Sponsors</Heading>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {[
              { title: "Core Sponsors", items: event.sponsors.core },
              { title: "Community Partners", items: event.sponsors.community },
              { title: "Platform", items: event.sponsors.platform },
            ].map((group) => (
              <article key={group.title} className="rounded-2xl p-5">
                <h3 className="text-2xl tracking-wide [font-family:var(--font-bebas-neue)]">
                  {group.title}
                </h3>

                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {group.items.map((sponsor) => (
                    <a
                      key={sponsor.name}
                      href={sponsor.href}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group p-3 transition bg-[#85011b] ring-1 ring-white/15 rounded-3xl"
                      aria-label={`Open ${sponsor.name}`}
                    >
                      <div className="flex items-center justify-center overflow-hidden px-3">
                        {sponsor.logo ? (
                          <img
                            src={sponsor.logo}
                            alt={`${sponsor.name} logo`}
                            className="h-16 w-auto object-contain"
                            loading="lazy"
                            referrerPolicy="no-referrer"
                          />
                        ) : (
                          <span className="text-md font-semibold text-black">
                            {sponsor.name}
                          </span>
                        )}
                      </div>
                      <p className="mt-2 text-center text-md font-bold text-white/85 leading-tight group-hover:text-white">
                        {sponsor.name}
                      </p>
                    </a>
                  ))}
                </div>
              </article>
            ))}
          </div>
        </section>

        <section className="space-y-4">
          <Heading>Rewards</Heading>
          <div className="grid pt-8 p-4 sm:p-6 gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
            {event.rewards.map((reward, idx) => (
              <article
                key={reward.title}
                className="group relative overflow-hidden rounded-3xl bg-[#85011b] p-5 ring-1 ring-white/15"
              >
                <div className="pointer-events-none absolute -right-8 -top-8 h-20 w-20 rounded-full bg-white/10 blur-2xl" />

                <div className="flex items-start justify-between gap-3">
                  <h3 className="text-xl md:text-2xl tracking-wide [font-family:var(--font-bebas-neue)] text-white">
                    {reward.title}
                  </h3>
                </div>

                <p className="mt-3 text-sm leading-relaxed text-white/85">
                  {reward.prize}
                </p>
              </article>
            ))}
          </div>
        </section>

        {/* <section className="space-y-4">
              <Heading>Contacts</Heading>
              <div className="rounded-2xl p-5 md:p-6">
                <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-1">
                  <div>
                    <p className="text-white/60">Email</p>
                    <a
                      href={`mailto:${event.contacts.email}`}
                      className="text-white underline-offset-2 hover:underline"
                    >
                      {event.contacts.email}
                    </a>
                  </div>
                </div>
              </div>
            </section> */}

        <section className="mt-10 px-6 md:mt-14">
          <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-[#85011b] px-5 py-8 md:px-10 md:py-12">
            <div className="pointer-events-none absolute -right-12 -top-12 h-40 w-40 rounded-full bg-white/10 blur-3xl" />
            <div className="pointer-events-none absolute -bottom-14 -left-14 h-44 w-44 rounded-full bg-black/35 blur-3xl" />

            <div className="relative z-10 mx-auto max-w-3xl text-center">
              <h2 className="mt-3 text-4xl md:text-6xl tracking-wide text-white [font-family:var(--font-special-agent)]">
                Register, compete, and claim your spot on the leaderboard.
              </h2>
              <p className="mt-4 text-sm md:text-base text-white/80">
                Team up, solve high-stakes challenges, and connect with the
                cybersecurity community at VIT Bhopal.
              </p>

              <div className="mt-7 flex flex-col items-center justify-center gap-3 sm:flex-row">
                <a
                  href={event.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`${ctaButtonClassName} text-base px-8 py-6 rounded-xl hover:ring-1 hover:ring-white/30`}
                >
                  Register on Unstop
                </a>
                <Link
                  href="/contact"
                  className="inline-flex h-12 items-center justify-center rounded-xl border border-white/25 px-7 text-sm font-semibold text-white transition hover:bg-white/10"
                >
                  Contact Organizers
                </Link>
              </div>
            </div>
          </div>
        </section>
      </Container>

      <footer
        className="relative py-4 md:py-8 bg-[#0F0F0F]"
        style={{
          backgroundImage: "url(/events/hackzero/hackzero-bg-overlay.png)",
          backgroundRepeat: "repeat",
          backgroundSize: "512px 512px",
          backgroundPosition: "0 0",
          backgroundAttachment: "fixed",
        }}
      >
        <Container className="px-4 md:px-6 lg:px-8">
          <div className=" px-5 md:px-10 pt-5 md:pt-10 pb-3 md:pb-6">
            {/* Main footer content */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-5 md:gap-8 lg:gap-10 mb-5 md:mb-8">
              {/* Brand Section */}
              <div className="col-span-1 sm:col-span-2 lg:col-span-1 flex flex-col gap-2">
                <div>
                  <svg
                    width="120"
                    height="43"
                    viewBox="0 0 3069 900"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-10 md:h-12 w-auto"
                  >
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M451.17 0.45979C488.767 1.44938 512.843 3.75843 535.269 8.7064C552.419 12.6648 579.793 20.2517 596.283 26.1892C612.443 31.7969 636.189 41.6928 649.051 47.9603C661.584 54.2277 684.67 68.082 700.171 78.3078C715.671 88.5336 741.396 109.315 757.556 124.819C773.717 140.322 796.143 165.392 807.686 180.896C819.229 196.399 836.049 223.118 844.954 240.271C853.859 257.424 865.732 284.803 871.668 301.296C877.275 317.459 884.86 345.828 888.818 363.97C893.765 388.38 895.414 409.492 895.414 446.436C895.744 477.444 893.765 507.132 890.467 525.604C887.499 541.767 880.902 568.486 875.626 584.979C870.349 601.143 859.795 627.202 852.539 642.706C845.284 658.209 832.092 681.63 823.187 694.495C814.612 707.689 798.452 729.13 787.568 741.995C776.685 754.86 755.907 775.311 741.396 787.846C726.885 800.051 704.458 816.874 691.926 824.791C679.063 833.038 659.275 844.583 647.402 850.52C635.529 856.458 614.752 865.364 601.23 870.312C587.708 875.26 565.941 881.857 553.409 884.496C540.546 887.465 514.822 892.083 495.693 895.052C476.565 897.691 455.127 900 447.872 900C440.616 900 419.179 897.691 400.05 895.052C380.922 892.083 354.867 887.465 342.335 884.496C329.473 881.857 308.036 875.26 294.514 870.312C280.992 865.364 259.225 856.128 246.692 849.531C233.83 843.263 210.084 829.079 193.924 818.523C177.434 807.638 150.06 784.547 132.91 767.395C115.761 750.242 93.0043 724.512 82.7804 709.998C72.5565 695.814 56.3962 669.095 47.1618 650.952C37.9273 632.81 26.0544 603.782 20.4478 586.629C15.1709 569.476 8.24506 541.767 5.27684 525.604C2.30862 507.461 0 477.444 0 448.086C0 417.738 1.97881 389.37 5.60664 370.568C8.57486 354.074 15.5007 326.696 21.1074 309.543C26.714 292.39 39.2465 262.702 49.1406 243.57C59.0346 224.438 76.5142 196.399 88.0573 180.896C99.6003 165.392 122.027 140.322 138.187 124.819C154.348 109.315 180.072 88.5336 195.573 78.3078C211.074 68.082 235.479 53.568 249.99 46.3109C264.502 39.0539 290.556 28.8281 307.706 23.2204C324.855 17.6127 351.569 10.6856 367.07 7.71681C382.571 4.74803 395.763 1.77925 396.752 0.789655C397.412 -0.199938 422.147 -0.199938 451.17 0.45979ZM345.633 83.5856C329.143 88.2037 301.769 98.7594 284.62 107.006C267.47 115.253 242.735 129.437 230.202 138.013C217.34 146.92 195.573 165.722 181.062 179.906C166.88 194.09 147.752 215.861 138.847 228.726C129.612 241.261 116.75 263.032 109.824 276.556C103.228 290.081 93.6639 311.522 89.0467 324.387C84.7592 336.921 78.493 359.352 75.8546 373.866C72.8864 388.38 69.5883 412.131 68.2691 426.645C67.2797 441.159 67.9393 467.878 69.9181 486.02C71.8969 504.163 76.844 530.882 80.8016 545.396C84.4294 559.91 93.0043 584.32 99.6003 599.823C106.196 615.327 120.378 641.056 131.261 657.55C141.815 673.713 163.582 700.102 179.083 715.606C194.583 731.109 218.329 751.231 231.851 760.467C245.373 769.374 266.151 781.909 278.023 787.846C289.896 793.784 311.993 803.02 327.494 808.298C342.995 813.576 368.719 819.843 385.209 822.482C401.37 825.121 429.733 827.43 447.872 827.43C466.011 827.43 494.044 825.121 510.534 822.482C526.695 819.843 552.749 813.576 568.25 808.298C583.75 803.02 605.847 793.784 617.72 787.846C629.593 781.909 649.381 770.033 662.243 761.457C674.776 752.551 694.234 737.707 704.788 727.811C715.671 718.245 733.151 699.113 744.034 685.588C754.918 672.064 769.429 651.282 776.025 639.407C782.951 627.532 793.505 606.091 799.111 591.577C805.048 577.063 812.633 553.972 815.931 540.448C819.559 526.923 823.847 500.864 825.825 482.722C828.464 458.971 828.464 440.169 825.825 415.099C823.847 395.967 818.57 366.279 813.953 349.126C809.335 331.974 797.462 301.296 787.568 281.504C777.674 261.383 762.503 235.653 753.599 223.778C744.364 211.903 726.885 192.441 714.682 179.906C702.15 167.701 683.021 151.208 672.138 142.961C661.254 134.715 641.136 122.18 627.614 115.253C614.092 107.996 592.325 98.0997 579.793 93.4816C566.93 88.5336 544.174 81.9363 528.673 78.3078C513.173 75.0091 482.831 71.3806 461.064 70.7209C439.297 69.7313 410.934 70.391 398.401 72.3702C385.539 74.0196 361.793 78.9675 345.633 83.5856ZM3007 512.511C3013.92 512.511 3023.49 514.161 3028.43 516.14C3033.71 517.789 3041.96 523.397 3047.23 528.345C3052.51 533.293 3059.44 543.189 3062.73 550.446C3066.03 557.703 3069 569.578 3069 576.835C3069 584.092 3066.03 595.967 3062.73 603.224C3059.44 610.481 3052.51 620.377 3047.23 625.325C3041.96 630.273 3033.71 635.88 3028.43 637.53C3023.49 639.509 3014.25 641.158 3007.99 641.158C3001.72 641.158 2992.49 639.509 2987.21 637.53C2982.26 635.88 2974.02 630.273 2968.74 625.325C2963.46 620.377 2956.54 610.481 2953.24 603.224C2949.94 595.967 2946.97 584.422 2946.97 577.495C2946.97 570.897 2949.28 559.682 2952.25 552.755C2954.89 546.157 2960.49 536.261 2964.45 530.654C2968.41 525.376 2976.98 519.108 2983.25 516.799C2989.52 514.49 3000.4 512.841 3007 512.511ZM2967.75 541.869C2963.79 547.477 2959.18 557.703 2958.19 564.3C2956.87 571.227 2956.87 582.113 2958.19 588.38C2959.18 594.647 2962.8 603.554 2966.43 608.172C2969.73 612.79 2976.66 619.387 2981.27 623.016C2987.87 627.634 2995.45 629.613 3007.99 629.613C3020.85 629.613 3028.1 627.634 3035.03 622.686C3039.98 618.728 3047.23 611.471 3050.86 606.852C3055.81 599.925 3057.46 592.668 3057.46 576.835C3057.46 561.001 3055.81 553.744 3050.86 546.817C3047.23 542.199 3040.64 535.272 3036.02 531.643C3031.4 528.015 3021.18 524.716 3012.93 523.727C3004.36 523.067 2993.47 524.386 2986.88 527.025C2980.61 529.334 2972.04 535.932 2967.75 541.869ZM1212.21 120.302C1216.83 120.302 1236.62 121.622 1256.74 123.271C1276.52 124.921 1301.92 129.539 1312.8 133.167C1323.69 137.126 1337.87 144.053 1343.8 149.001C1350.07 153.949 1358.31 164.504 1361.94 172.751C1368.54 186.605 1368.87 194.522 1369.86 307.995C1370.52 380.895 1369.53 434.993 1367.55 444.889C1365.57 453.795 1360.62 466.66 1356.34 472.927C1352.05 479.195 1343.8 487.771 1337.54 492.06C1331.27 496.348 1318.41 501.956 1309.5 504.594C1300.27 507.233 1277.18 510.862 1257.39 512.511C1233.98 514.82 1209.24 514.82 1184.18 512.511C1162.74 510.532 1138.01 506.244 1127.12 502.615C1116.9 498.657 1103.71 492.389 1098.43 488.101C1092.82 484.143 1084.91 473.257 1072.05 448.188V187.595L1080.29 171.431C1084.91 162.525 1092.82 151.64 1098.43 147.681C1103.71 143.393 1117.23 137.126 1128.11 133.167C1139 129.539 1160.43 125.25 1175.93 123.601C1191.43 121.952 1207.59 120.632 1212.21 120.302ZM1132.07 178.359C1127.12 183.307 1121.52 193.532 1119.87 200.789C1118.22 208.046 1116.57 260.825 1116.57 317.891C1116.57 374.958 1118.22 427.736 1119.87 434.993C1121.52 442.91 1127.12 452.146 1133.72 458.413C1141.3 465.67 1150.54 469.959 1164.39 472.598C1175.27 474.907 1201.33 476.886 1222.11 476.886C1242.88 476.556 1267.29 474.907 1276.52 472.927C1285.43 470.948 1297.3 466.66 1302.25 463.691C1307.19 460.723 1314.45 452.146 1318.08 444.889C1324.01 432.354 1324.34 425.427 1323.69 314.593C1322.7 204.088 1322.37 196.831 1316.1 186.605C1312.47 180.668 1302.91 172.751 1294.66 168.463C1281.47 161.865 1273.88 161.206 1227.05 160.546C1189.46 159.886 1169.67 161.206 1157.79 164.504C1148.56 167.143 1137.35 173.411 1132.07 178.359ZM2138.96 122.282C2158.09 123.271 2182.82 125.91 2194.03 128.549C2205.58 131.188 2220.42 135.806 2227.01 139.435C2233.94 142.733 2244.49 150.98 2250.1 157.577C2256.04 164.174 2262.3 175.06 2264.28 181.657C2266.26 189.244 2267.58 257.856 2267.58 509.213H2221.41V367.371H2016.93V509.213H1970.76L1971.75 349.888C1972.74 221.571 1973.73 188.254 1977.68 177.699C1979.99 170.442 1987.58 159.226 1993.84 152.629C2001.43 145.042 2012.97 138.445 2026.82 133.827C2038.7 129.868 2060.79 125.25 2076.29 123.601C2091.8 121.952 2119.83 121.292 2138.96 122.282ZM2032.43 178.359C2027.15 183.966 2022.21 193.862 2020.23 203.098C2018.25 212.005 2016.93 243.342 2016.93 273.36V327.787H2221.41C2221.41 210.355 2220.42 199.14 2215.47 189.244C2212.17 182.317 2204.26 174.4 2195.02 169.452C2181.17 161.865 2176.88 161.206 2125.76 160.546C2084.87 159.886 2067.72 160.876 2056.51 164.504C2048.26 167.143 2037.71 173.411 2032.43 178.359ZM2488.55 123.601C2519.55 123.601 2547.91 125.25 2561.1 127.889C2572.98 130.198 2588.48 135.146 2595.73 138.775C2602.99 142.403 2613.87 150.98 2619.48 157.577C2625.42 164.174 2631.68 175.06 2633.66 181.657C2635.31 188.584 2636.96 204.088 2636.96 238.724H2590.79V219.592C2590.79 208.376 2588.48 196.171 2584.85 189.244C2580.89 181.327 2573.97 175.06 2562.75 169.452C2547.25 161.535 2543.29 161.206 2490.2 161.206C2437.1 161.206 2433.14 161.535 2417.64 169.452C2407.09 174.73 2398.84 181.987 2394.55 189.244C2389.61 198.15 2388.29 207.057 2387.96 228.828C2387.96 250.269 2389.28 259.175 2393.56 266.103C2396.86 271.05 2403.46 276.988 2408.41 279.627C2413.68 282.266 2453.92 290.512 2498.44 297.769C2563.41 308.325 2582.87 312.943 2597.05 320.2C2608.93 326.138 2618.82 334.384 2635.31 358.135V406.295C2635.31 450.167 2634.65 456.104 2627.72 468.639C2623.11 477.546 2614.53 486.452 2604.64 492.719C2596.06 498.327 2579.24 504.924 2566.71 507.563C2554.51 510.202 2530.1 513.501 2512.29 514.49C2493.49 515.81 2466.12 515.15 2445.67 512.511C2426.54 510.532 2402.8 505.584 2392.9 501.626C2383.01 497.997 2369.49 489.421 2363.22 483.153C2356.3 476.226 2349.7 465.011 2347.39 456.434C2344.75 448.188 2342.77 428.066 2341.79 382.215L2386.31 380.565V394.42C2386.31 402.336 2387.96 417.51 2389.61 428.396C2391.92 440.931 2395.54 451.156 2400.49 455.775C2404.45 460.063 2411.7 465.67 2416.65 468.309C2421.93 470.948 2439.74 474.577 2456.23 476.556C2473.05 478.535 2498.11 479.195 2511.63 477.875C2525.16 476.886 2543.62 473.917 2552.86 471.278C2562.75 468.639 2573.31 462.702 2579.24 456.434C2588.48 446.538 2589.14 444.559 2590.13 414.212C2591.12 387.822 2590.46 380.236 2585.18 371.329C2581.88 365.062 2573.64 357.805 2566.05 354.176C2558.47 350.548 2521.86 342.961 2478.65 336.034C2436.11 329.107 2398.18 321.19 2389.61 317.561C2381.36 313.933 2370.48 307.006 2365.53 302.058C2360.25 297.11 2353 287.214 2349.04 279.957C2343.1 268.741 2342.45 261.155 2342.45 227.178C2342.77 190.234 2343.43 186.275 2352.34 169.782C2359.26 156.258 2365.53 149.66 2378.06 142.733C2386.97 137.455 2404.12 131.188 2415.99 128.549C2429.18 125.58 2457.22 123.601 2488.55 123.601ZM1446.37 126.57H1492.54V282.266C1492.54 428.396 1492.87 438.951 1498.81 449.837C1502.11 456.104 1510.02 464.351 1515.96 467.979C1525.52 473.587 1533.11 474.577 1568.4 474.577C1603.69 474.577 1611.27 473.587 1620.84 467.979C1627.1 464.351 1635.35 456.104 1638.98 449.837C1645.57 438.622 1645.9 433.344 1647.22 126.57H1694.05L1694.71 284.575C1695.37 439.611 1695.37 442.91 1702.3 452.146C1706.26 457.094 1714.17 464.351 1720.44 467.979C1730 473.587 1737.59 474.577 1772.88 474.577C1809.15 474.577 1815.42 473.587 1825.64 467.65C1832.9 463.361 1839.5 455.445 1842.79 446.868C1848.07 434.663 1848.73 418.83 1848.73 126.57H1894.9V282.266C1894.9 411.573 1894.24 441.26 1889.96 455.445C1887.32 465.011 1880.72 477.875 1875.77 483.483C1870.5 489.421 1858.62 497.667 1849.39 501.626C1839.83 505.914 1819.38 510.532 1803.22 512.511C1786.4 514.49 1764.96 514.49 1751.44 512.841C1738.58 511.522 1720.11 506.574 1710.21 501.956C1700.32 497.337 1687.13 488.761 1670.64 471.278L1658.1 483.483C1651.51 490.08 1639.31 498.657 1631.06 502.285C1622.82 505.914 1605.34 510.532 1592.14 512.511C1575.98 514.82 1558.83 514.82 1538.72 512.511C1521.9 510.532 1501.45 505.914 1491.88 501.626C1481.99 497.337 1470.45 489.091 1463.85 481.174C1457.91 473.917 1451.65 461.712 1449.67 453.795C1447.69 445.549 1446.37 377.597 1446.37 283.255V126.57ZM2712.81 126.57H2813.4C2875.41 126.57 2920.59 127.889 2931.14 130.198C2940.71 132.178 2955.22 137.455 2963.46 141.414C2971.71 145.702 2981.6 152.629 2985.56 156.917C2989.52 160.876 2996.11 171.761 3000.4 180.997C3007.66 196.501 3007.99 201.449 3007.99 263.464C3007.99 323.499 3007.33 330.756 3001.06 344.28C2997.43 352.527 2989.19 364.072 2982.92 370.34C2976.66 376.607 2965.11 384.524 2956.87 387.822C2948.62 391.121 2933.12 395.409 2922.24 397.059C2911.35 399.038 2870.13 400.357 2758.99 400.357V509.213H2712.81V126.57ZM2759.32 263.134L2758.99 364.072C2869.8 364.072 2904.43 362.423 2914.65 360.114C2924.22 358.135 2936.42 352.527 2941.7 347.909C2946.97 343.291 2953.57 334.714 2956.54 329.436C2960.82 321.52 2961.81 308.655 2961.81 263.464C2961.81 213.984 2961.15 206.067 2955.22 194.522C2950.93 186.605 2943.02 178.359 2933.78 173.081C2918.94 164.504 2918.28 164.504 2839.13 163.515L2759.32 162.525V263.134ZM618.38 183.205C621.678 188.153 625.306 197.389 626.625 203.656C627.614 210.913 626.625 220.479 618.05 243.24L651.03 276.226L662.573 271.278C668.839 268.64 678.404 266.331 684.01 266.331C689.287 266.331 698.852 268.969 705.448 272.268C711.714 275.237 716.991 279.195 716.991 280.515C716.991 281.834 716.661 282.824 716.001 283.154C715.671 283.154 708.086 280.185 699.511 276.556C690.936 272.598 682.032 270.289 679.723 271.278C677.414 272.268 671.478 274.247 657.626 279.525L664.222 288.761C667.85 293.709 670.818 299.647 670.818 301.956C670.818 304.265 664.552 312.512 656.637 320.758C649.051 328.675 638.168 336.921 632.891 338.901C627.284 340.88 620.359 341.869 617.06 340.88C614.092 339.89 608.815 338.241 605.517 337.251C600.57 335.932 599.911 338.241 600.241 354.734C600.9 372.547 601.56 374.196 613.433 382.773C624.316 390.359 628.604 391.349 641.796 390.03C651.03 389.04 657.626 389.7 657.626 391.679C657.626 393.658 654.328 395.967 650.041 396.627C646.083 397.617 637.838 398.276 631.902 398.276C625.965 398.276 616.071 396.297 609.805 393.328C603.209 390.689 598.262 389.37 598.262 390.03C598.262 391.019 600.241 395.967 602.549 400.915C606.177 407.842 612.113 411.471 627.944 416.419C639.157 420.047 649.051 424.336 649.711 425.655C650.37 426.974 649.051 427.964 646.743 427.964C644.434 427.964 633.88 425.985 622.997 423.346C612.113 420.707 601.89 418.398 600.57 418.398C599.251 418.398 598.262 420.047 598.262 422.356C598.262 424.665 605.188 433.572 613.762 442.148C622.337 450.725 628.604 458.312 627.944 459.301C626.955 459.961 618.38 454.353 608.815 446.766C595.294 436.211 589.687 428.954 584.74 415.759C581.112 406.193 576.825 398.276 575.176 398.276C573.197 398.276 571.878 400.255 571.878 402.564C571.548 404.874 573.856 418.398 576.495 432.912C579.463 447.426 585.399 484.701 589.687 515.378C594.304 546.385 599.581 596.855 601.56 627.532C603.539 664.147 603.539 697.793 601.56 723.193C599.911 744.964 597.272 764.096 595.953 765.415C594.304 767.065 588.038 762.447 581.442 755.519C575.176 748.592 563.303 732.099 555.387 718.245C547.472 704.72 535.929 683.279 529.993 670.414C524.056 657.88 517.13 635.779 514.162 621.924C511.524 607.74 509.215 586.959 509.215 575.743C509.215 558.26 511.194 551.993 521.418 533.521C532.961 513.069 533.62 509.441 538.897 457.652C542.195 427.964 543.514 402.235 542.525 401.245C541.536 399.926 536.259 397.946 531.312 396.957C522.737 395.307 522.407 395.637 524.386 405.863C526.035 413.45 524.056 421.697 518.12 434.561C513.832 444.787 498.332 466.228 484.15 482.392C469.639 498.555 449.191 519.337 438.308 528.243C427.424 537.149 407.966 551.333 395.433 559.58C382.571 568.156 349.261 586.299 321.228 600.483C292.865 614.667 269.119 625.223 268.459 624.233C267.47 623.244 277.364 601.473 290.226 575.743C303.088 549.684 324.196 513.069 336.728 493.937C353.218 469.197 370.698 449.075 397.742 423.016C419.179 402.564 442.925 382.773 453.149 377.495C467 369.908 474.586 368.259 485.469 369.248C498.991 370.238 499.321 369.908 497.342 361.991C496.353 357.703 494.374 352.755 493.055 351.436C492.065 350.116 467.99 351.436 439.957 354.074C411.594 356.713 385.869 360.012 382.241 361.002C378.613 361.991 368.06 367.269 359.155 372.877C348.931 378.814 336.399 383.432 326.175 384.422C316.94 385.082 295.503 383.432 278.353 380.464C257.246 376.835 237.788 370.568 218.989 361.661C203.488 354.404 180.402 342.199 167.869 334.283C155.007 326.366 140.166 315.15 134.889 309.543C129.283 303.935 125.655 298.657 126.644 297.997C127.304 297.008 141.485 294.699 157.975 292.72C176.115 290.411 208.435 289.751 242.075 291.07C271.757 292.39 314.302 295.359 336.069 297.997C357.836 300.307 402.029 306.904 434.02 312.512C466.341 318.119 493.385 322.407 494.374 322.407C495.034 322.407 496.023 321.418 496.023 320.098C496.023 318.779 487.118 313.171 475.905 307.564C462.383 300.966 452.159 293.05 444.574 282.824C438.308 274.907 434.02 267.32 435.01 266.331C435.999 265.671 443.584 271.938 452.159 280.515C460.734 289.091 469.639 296.018 471.947 296.018C474.256 296.018 475.575 293.709 474.586 290.411C473.596 287.112 470.628 275.237 467.99 264.021C465.351 252.476 464.032 243.24 465.351 243.24C466.671 243.24 469.639 247.198 471.618 252.146C473.926 257.424 477.224 266.99 479.203 273.917C481.512 282.494 485.799 287.772 492.725 291.07C498.002 294.039 502.619 294.699 502.619 292.72C502.619 291.07 500.64 286.122 498.661 282.164C496.683 277.876 495.034 268.64 495.363 261.383C495.693 254.126 496.683 244.23 497.672 239.282C498.332 234.334 500.31 230.045 501.63 230.045C502.949 230.045 503.608 238.622 502.949 249.178C502.289 265.011 503.279 269.959 510.534 280.515C518.45 292.06 520.099 293.05 555.717 294.369L553.738 280.515C552.749 271.278 553.738 262.372 557.036 254.785C559.675 248.518 563.303 243.24 565.281 243.24C566.931 243.24 572.537 238.952 577.484 233.344C582.431 227.736 589.357 223.448 592.325 223.448C595.623 223.448 601.23 226.417 604.858 230.045C608.486 233.674 611.784 236.973 612.113 236.973C612.773 236.973 615.082 232.025 617.72 226.087C620.359 220.149 622.337 212.233 622.337 208.604C622.337 204.976 619.039 196.399 615.082 189.802C611.124 182.875 609.145 176.607 610.135 175.618C611.454 174.628 615.082 178.257 618.38 183.205ZM2983.58 538.57H2999.08C3009.97 538.57 3017.55 540.55 3023.82 545.168C3029.42 549.456 3033.05 555.064 3033.05 560.012C3033.05 564.63 3030.74 570.237 3028.43 572.547C3024.48 575.845 3024.48 577.494 3028.43 583.102C3031.07 586.731 3033.05 595.307 3033.05 602.234C3033.05 612.13 3031.73 614.439 3027.11 613.78C3024.15 613.12 3020.85 609.821 3020.52 606.193C3019.86 602.564 3018.87 596.297 3017.88 592.338C3016.89 586.731 3014.25 584.751 3006.67 584.751C2997.1 584.751 2996.77 585.411 2996.77 599.595C2996.77 612.46 2995.78 614.439 2990.18 614.439C2983.91 614.439 2983.58 612.46 2983.58 576.505V538.57ZM3004.03 571.557C3008.32 571.557 3013.59 570.567 3015.9 569.248C3018.21 567.928 3019.86 563.64 3019.2 560.012C3018.21 555.394 3014.91 553.085 3007.66 552.425C2997.76 551.435 2997.1 552.095 2996.77 561.661C2996.77 569.908 2998.09 571.557 3004.03 571.557Z"
                      fill="currentColor"
                    ></path>
                    <path
                      d="M1363.39 793C1361.91 793 1360.95 792.26 1360.5 790.78L1313.88 640.708L1313.66 639.82C1313.66 638.34 1314.47 637.6 1316.1 637.6H1338.08C1339.71 637.6 1340.74 638.34 1341.19 639.82L1374.49 755.704C1374.64 756.148 1374.86 756.37 1375.15 756.37C1375.45 756.37 1375.67 756.148 1375.82 755.704L1408.9 639.82C1409.34 638.34 1410.38 637.6 1412.01 637.6H1433.54C1434.43 637.6 1435.09 637.896 1435.54 638.488C1435.98 639.08 1436.06 639.82 1435.76 640.708L1388.47 790.78C1388.03 792.26 1387.07 793 1385.59 793H1363.39ZM1483.57 793C1482.83 793 1482.16 792.778 1481.57 792.334C1481.13 791.742 1480.9 791.076 1480.9 790.336V640.264C1480.9 639.524 1481.13 638.932 1481.57 638.488C1482.16 637.896 1482.83 637.6 1483.57 637.6H1503.99C1504.73 637.6 1505.32 637.896 1505.77 638.488C1506.36 638.932 1506.66 639.524 1506.66 640.264V790.336C1506.66 791.076 1506.36 791.742 1505.77 792.334C1505.32 792.778 1504.73 793 1503.99 793H1483.57ZM1663.33 637.6C1664.07 637.6 1664.66 637.896 1665.1 638.488C1665.69 638.932 1665.99 639.524 1665.99 640.264V657.358C1665.99 658.098 1665.69 658.764 1665.1 659.356C1664.66 659.8 1664.07 660.022 1663.33 660.022H1622.48C1621.74 660.022 1621.37 660.392 1621.37 661.132V790.336C1621.37 791.076 1621.07 791.742 1620.48 792.334C1620.04 792.778 1619.44 793 1618.7 793H1598.28C1597.54 793 1596.87 792.778 1596.28 792.334C1595.84 791.742 1595.62 791.076 1595.62 790.336V661.132C1595.62 660.392 1595.25 660.022 1594.51 660.022H1555.21C1554.47 660.022 1553.81 659.8 1553.21 659.356C1552.77 658.764 1552.55 658.098 1552.55 657.358V640.264C1552.55 639.524 1552.77 638.932 1553.21 638.488C1553.81 637.896 1554.47 637.6 1555.21 637.6H1663.33ZM1865.41 711.97C1864.52 712.414 1864.52 712.858 1865.41 713.302C1880.51 720.406 1888.06 732.616 1888.06 749.932C1888.06 763.992 1883.32 774.722 1873.85 782.122C1864.52 789.374 1852.31 793 1837.22 793H1781.27C1780.53 793 1779.87 792.778 1779.28 792.334C1778.83 791.742 1778.61 791.076 1778.61 790.336V640.264C1778.61 639.524 1778.83 638.932 1779.28 638.488C1779.87 637.896 1780.53 637.6 1781.27 637.6H1835.44C1851.57 637.6 1864.01 641.078 1872.74 648.034C1881.47 654.99 1885.84 665.498 1885.84 679.558C1885.84 695.098 1879.03 705.902 1865.41 711.97ZM1805.47 659.8C1804.73 659.8 1804.36 660.17 1804.36 660.91V701.98C1804.36 702.72 1804.73 703.09 1805.47 703.09H1835.44C1843.29 703.09 1849.43 701.24 1853.87 697.54C1858.46 693.692 1860.75 688.438 1860.75 681.778C1860.75 674.97 1858.46 669.642 1853.87 665.794C1849.43 661.798 1843.29 659.8 1835.44 659.8H1805.47ZM1837.22 770.578C1844.91 770.578 1850.98 768.506 1855.42 764.362C1860.01 760.218 1862.3 754.594 1862.3 747.49C1862.3 740.238 1860.01 734.614 1855.42 730.618C1850.83 726.474 1844.62 724.402 1836.77 724.402H1805.47C1804.73 724.402 1804.36 724.772 1804.36 725.512V769.468C1804.36 770.208 1804.73 770.578 1805.47 770.578H1837.22ZM2023.49 640.264C2023.49 639.524 2023.71 638.932 2024.16 638.488C2024.75 637.896 2025.42 637.6 2026.16 637.6H2046.58C2047.32 637.6 2047.91 637.896 2048.36 638.488C2048.95 638.932 2049.24 639.524 2049.24 640.264V790.336C2049.24 791.076 2048.95 791.742 2048.36 792.334C2047.91 792.778 2047.32 793 2046.58 793H2026.16C2025.42 793 2024.75 792.778 2024.16 792.334C2023.71 791.742 2023.49 791.076 2023.49 790.336V726.844C2023.49 726.104 2023.12 725.734 2022.38 725.734H1965.77C1965.03 725.734 1964.66 726.104 1964.66 726.844V790.336C1964.66 791.076 1964.37 791.742 1963.77 792.334C1963.33 792.778 1962.74 793 1962 793H1941.57C1940.83 793 1940.17 792.778 1939.58 792.334C1939.13 791.742 1938.91 791.076 1938.91 790.336V640.264C1938.91 639.524 1939.13 638.932 1939.58 638.488C1940.17 637.896 1940.83 637.6 1941.57 637.6H1962C1962.74 637.6 1963.33 637.896 1963.77 638.488C1964.37 638.932 1964.66 639.524 1964.66 640.264V702.424C1964.66 703.164 1965.03 703.534 1965.77 703.534H2022.38C2023.12 703.534 2023.49 703.164 2023.49 702.424V640.264ZM2155.07 795.22C2143.97 795.22 2134.13 793 2125.55 788.56C2116.96 783.972 2110.3 777.534 2105.57 769.246C2100.98 760.958 2098.69 751.412 2098.69 740.608V689.992C2098.69 679.336 2101.05 669.938 2105.79 661.798C2110.53 653.51 2117.11 647.146 2125.55 642.706C2134.13 638.118 2143.97 635.824 2155.07 635.824C2166.32 635.824 2176.16 638.118 2184.6 642.706C2193.18 647.146 2199.84 653.51 2204.58 661.798C2209.32 669.938 2211.68 679.336 2211.68 689.992V740.608C2211.68 751.412 2209.32 760.958 2204.58 769.246C2199.84 777.534 2193.18 783.972 2184.6 788.56C2176.16 793 2166.32 795.22 2155.07 795.22ZM2155.07 773.02C2164.4 773.02 2171.87 770.134 2177.5 764.362C2183.12 758.59 2185.93 750.968 2185.93 741.496V689.77C2185.93 680.15 2183.12 672.454 2177.5 666.682C2171.87 660.91 2164.4 658.024 2155.07 658.024C2145.9 658.024 2138.5 660.91 2132.87 666.682C2127.25 672.454 2124.44 680.15 2124.44 689.77V741.496C2124.44 750.968 2127.25 758.59 2132.87 764.362C2138.5 770.134 2145.9 773.02 2155.07 773.02ZM2324.93 637.378C2334.25 637.378 2342.46 639.302 2349.57 643.15C2356.67 646.998 2362.15 652.474 2366 659.578C2369.99 666.534 2371.99 674.6 2371.99 683.776C2371.99 692.804 2369.99 700.796 2366 707.752C2362 714.708 2356.3 720.11 2348.9 723.958C2341.65 727.658 2333.21 729.508 2323.59 729.508H2288.07C2287.33 729.508 2286.96 729.878 2286.96 730.618V790.336C2286.96 791.076 2286.67 791.742 2286.08 792.334C2285.63 792.778 2285.04 793 2284.3 793H2263.88C2263.14 793 2262.47 792.778 2261.88 792.334C2261.43 791.742 2261.21 791.076 2261.21 790.336V640.042C2261.21 639.302 2261.43 638.71 2261.88 638.266C2262.47 637.674 2263.14 637.378 2263.88 637.378H2324.93ZM2321.15 708.64C2328.7 708.64 2334.77 706.42 2339.36 701.98C2343.94 697.54 2346.24 691.694 2346.24 684.442C2346.24 677.042 2343.94 671.048 2339.36 666.46C2334.77 661.872 2328.7 659.578 2321.15 659.578H2288.07C2287.33 659.578 2286.96 659.948 2286.96 660.688V707.53C2286.96 708.27 2287.33 708.64 2288.07 708.64H2321.15ZM2503.36 793C2501.88 793 2500.92 792.26 2500.48 790.78L2493.15 767.026C2492.86 766.434 2492.49 766.138 2492.04 766.138H2433.43C2432.99 766.138 2432.62 766.434 2432.32 767.026L2425 790.78C2424.55 792.26 2423.59 793 2422.11 793H2399.91C2399.02 793 2398.36 792.778 2397.91 792.334C2397.47 791.742 2397.4 790.928 2397.69 789.892L2445.87 639.82C2446.31 638.34 2447.27 637.6 2448.75 637.6H2476.5C2477.98 637.6 2478.94 638.34 2479.39 639.82L2527.78 789.892C2527.93 790.188 2528.01 790.558 2528.01 791.002C2528.01 792.334 2527.19 793  2525.56 793H2503.36ZM2439.21 744.604C2439.06 745.492 2439.35 745.936 2440.09 745.936H2485.16C2486.05 745.936 2486.34 745.492 2486.05 744.604L2463.18 670.234C2463.03 669.642 2462.81 669.346 2462.52 669.346C2462.22 669.346 2462 669.642 2461.85 670.234L2439.21 744.604ZM2575.83 793C2575.09 793 2574.43 792.778 2573.84 792.334C2573.39 791.742 2573.17 791.076 2573.17 790.336V640.264C2573.17 639.524 2573.39 638.932 2573.84 638.488C2574.43 637.896 2575.09 637.6 2575.83 637.6H2596.26C2597 637.6 2597.59 637.896 2598.03 638.488C2598.63 638.932 2598.92 639.524 2598.92 640.264V769.69C2598.92 770.43 2599.29 770.8 2600.03 770.8H2674.4C2675.14 770.8 2675.73 771.096 2676.18 771.688C2676.77 772.132 2677.07 772.724 2677.07 773.464V790.336C2677.07 791.076 2676.77 791.742 2676.18 792.334C2675.73 792.778 2675.14 793 2674.4 793H2575.83Z"
                      fill="currentColor"
                    ></path>
                  </svg>
                </div>
                <p className="text-white/60 text-sm leading-relaxed">
                  VIT Bhopal&apos;s premier cybersecurity community dedicated to
                  learning and innovation.
                </p>
              </div>

              {/* Quick Links */}
              <div>
                <h4 className="text-white font-semibold mb-2 md:mb-4 text-sm md:text-base">
                  Quick Links
                </h4>
                <nav className="flex flex-col gap-1.5 md:gap-2">
                  {footerItems.map((item, idx) => (
                    <a
                      key={idx}
                      href={item.href}
                      className="text-white/60 hover:text-white text-sm transition-colors"
                    >
                      {item.name}
                    </a>
                  ))}
                </nav>
              </div>

              {/* Resources */}
              <div>
                <h4 className="text-white font-semibold mb-2 md:mb-4 text-sm md:text-base">
                  Resources
                </h4>
                <nav className="flex flex-col gap-1.5 md:gap-2">
                  {extraFooterItems.map((item, idx) => (
                    <a
                      key={idx}
                      href={item.href}
                      className="text-white/60 hover:text-white text-sm transition-colors"
                    >
                      {item.name}
                    </a>
                  ))}
                </nav>
              </div>

              {/* Get in Touch */}
              <div className="col-span-1 sm:col-span-2 lg:col-span-1">
                <h4 className="text-white font-semibold mb-2 md:mb-4 text-sm md:text-base">
                  Get in Touch
                </h4>
                <a
                  href="mailto:owaspclub@vitbhopal.ac.in"
                  className="text-white/80 hover:text-white text-sm block mb-3 md:mb-4 transition-colors"
                >
                  owaspclub@vitbhopal.ac.in
                </a>

                {/* Social Icons */}
                <div className="flex flex-wrap gap-2 md:gap-3 mt-2 md:mt-4">
                  {socialMediaLinks.map((item, idx) => (
                    <a
                      key={idx}
                      href={item.href}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-9 h-9 md:w-10 md:h-10 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20 flex items-center justify-center transition-all"
                      aria-label={item.name}
                    >
                      <SocialIcon name={item.name} size={18} />
                    </a>
                  ))}
                </div>
              </div>
            </div>

            {/* Bottom Bar */}
            <div className="border-t border-white/10 pt-4 md:pt-6 flex flex-col md:flex-row justify-between items-center gap-2 md:gap-4 text-xs md:text-sm text-white/40">
              <p>
                © {new Date().getFullYear()} OWASP VIT Bhopal. All rights
                reserved.
              </p>
              <div className="flex gap-6">
                <a
                  href="/privacy-policy"
                  className="hover:text-white/60 transition-colors"
                >
                  Privacy Policy
                </a>
              </div>
            </div>
          </div>
        </Container>
      </footer>
    </div>
  );
}
